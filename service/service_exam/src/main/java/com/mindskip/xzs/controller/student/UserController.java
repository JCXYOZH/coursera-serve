...
import com.mindskip.xzs.repository.StudentMemberMapper;   // 新增
...
public class UserController extends BaseApiController {
    ...
    private final StudentMemberMapper studentMemberMapper;   // 新增
    ...
    public UserController(UserService userService, UserEventLogService userEventLogService,
                          ...
                          StudentMemberMapper studentMemberMapper) {   // 新增参数
        this.userService = userService;
        this.userEventLogService = userEventLogService;
        this.messageService = messageService;
        this.authenticationService = authenticationService;
        this.eventPublisher = eventPublisher;
        this.studentMemberMapper = studentMemberMapper;   // 注入
    }

    /**
     * 校验手机号是否在知新堂注册（查询 student_member 表）
     */
    @GetMapping("/checkMobile/{mobile}")
    public RestResponse checkMobile(@PathVariable String mobile) {
        int count = studentMemberMapper.countByMobile(mobile);
        boolean isRegistered = count > 0;
        return RestResponse.ok(isRegistered);
    }
    ...
    public RestResponse register(@RequestBody @Valid UserRegisterVM model) {
        ...
        userEventLog.setContent("欢迎 " + user.getUserName() + " 注册来到温故阁");
