...
public class MemberServiceImpl extends ServiceImpl<MemberMapper, Member> implements MemberService {
    ...
    public void userRegister(RegisterVO registerVO) {
        ...
        member.setAvatar("https://ts1.tc.mm.bing.net/th/id/R-C.de033bc27a3a53b323039a41a49dd807?rik=yV4HXrkU6pZG%2fA&riu=http%3a%2f%2fimg95.699pic.com%2felement%2f40208%2f1829.png_300.png&ehk=nTm7Hg7mv9qKJp%2b%2baCEAjrD5Hx111iqxAdMPy8QoO2Y%3d&risl=&pid=ImgRaw&r=0");
     ...
     * 修改密码（支持修改手机号和密码）
     *
     * @param passwordVO 包含手机号、验证码、新密码
     * @param userMember 当前登录用户信息
     */
    @Override
    ...
    public void updatePassword(PasswordVO passwordVO, Member userMember) {
        ...
        // 2. 获取当前登录用户信息
        Member member = baseMapper.selectById(userMember.getId());
        if (member == null) {
            throw new OESException("用户不存在");
        }

        // 3. 如果需要修改手机号（输入的手机号与当前不同）
        if (!mobile.equals(member.getMobile())) {
            ...
            if (count > 0) {
                throw new OESException("新手机号已被占用");
            }
            member.setMobile(mobile);
        }
        ...
        if (StringUtils.hasText(newPassword)) {
            member.setPassword(MD5Util.getMD5(newPassword));
        }

        // 5. 保存更新
        int result = baseMapper.updateById(member);
        if (result < 1) {
            throw new OESException("修改失败");
        }

        // 6. 修改成功后，清除Redis中的验证码（防止重复使用）
