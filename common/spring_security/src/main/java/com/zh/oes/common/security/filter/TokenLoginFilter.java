...
//        redisTemplate.opsForValue().set(user.getCurrentUserInfo().getUsername(), user.getPermissionValueList());
        try {
            // redis设置用户缓存; K,V为用户名和用户的权限列表
// 假设 redisTemplate 已注入且 key/value 合适
            redisTemplate.opsForValue().set(user.getCurrentUserInfo().getUsername(), user.getPermissionValueList());
        } catch (Exception ex) {
// 记录但不要再向 response 写内容（response 可能已经写）
            log.error("redis write fail, continue login. user={}, error={}", user.getCurrentUserInfo().getUsername(), ex.getMessage(), ex);
        }
// 始终返回 token（或按照你的 Result.success 结构）
